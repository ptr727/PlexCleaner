...
- **Docker Hub README via m4 render.** `Docker/README.md` is generated from `Docker/README.m4`; the m4 render command is passed to the canonical `publish-docker-readme-task.yml` via its optional `transform-run` input, with `transform-artifact: versions-main` so the task downloads the tool-versions artifact before rendering - no bespoke build step.
- **.NET project list.** CODESTYLE.md carries the template's generic project-name placeholders. PlexCleaner ships a single `PlexCleaner` library/console and a `PlexCleanerTests` test project, and no Python side; `InternalsVisibleTo` grants test access to `PlexCleanerTests` (the CODESTYLE `YourTestProject`/`YourBenchmarkProject` placeholders map to it).
