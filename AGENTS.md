...
- **Distribution respects the user: pull by default.** The Docker image and the GitHub release binaries are **pull** channels - the user decides when to consume them. Because a release still reaches every user who pulls next, releasing is a deliberate act that marks a real functional change, never mechanical churn. This is why a **human merge never publishes** - a release is a deliberate `workflow_dispatch`, or the weekly schedule that refreshes the image against its base - together with the no-op republish guarantee and maintainer-gated version bumps: a needless release spends the user's attention.
- **Both branches stay in sync, so a promotion never needs a back-merge.** Dependabot targets `develop` and `main` in parallel, so neither branch drifts and a `develop -> main` promotion stays a clean forward merge by default. That is exactly what lets the model be **signed, linear, and free of back-merges** - forward sync removes any need to merge `main` back into `develop`, which the rules forbid. If sync is ever broken (a change lands on one branch only, or normalizes a file on one side), restore it forward-only, never back-merge. See "Branching Model".
...
- **Contracts state what, not how, and favor reuse.** [`WORKFLOW.md`](./WORKFLOW.md) fixes required outcomes, not a required implementation - a guarantee may be satisfied by more than one shape of YAML. Within that freedom, apply good engineering practice: minimize duplication and maximize reuse, which is why the pipeline splits a generic orchestration layer from a repo-owned build layer.
...
This repo is audited against the standard it declares, and conformance is a definition of done rather than a background task.

- **Definition of done is the audit.** A conformance change is complete only when the repo passes [`AUDIT.md`](./AUDIT.md), **or** carries a tracking issue recording the residual deltas. Do not leave a change partially applied and unrecorded - that is itself a defect.
- **Fix a rule discrepancy at its source, not locally.** When a rule this repo carries reads wrong in its context - a cross-reference to something this repo does not have, or an assumption it does not meet - do not quietly edit the carried text here. Report the discrepancy so the source is corrected, then take the correction. A local edit forks the text, and the next audit flags the fork as divergence.
...
- **Cross-platform by default - Windows + macOS + Linux.** Linux runs natively (a Linux desktop, or SSH/remote into a Linux host), or through WSL2 on Windows - the WSL route carries its own nuances (mounts, path translation, SSH-agent forwarding) but delivers the same toolchain. Editing is cross-platform through the GUI regardless of where code runs. This repo ships executables for all three, so all three are supported.
- **A repo's platform ceiling is set by its dependencies, not tooling effort.** Narrow below the default only for a hard runtime ceiling - the code can only execute or test on one platform. The narrowing axis is where code *executes* for dev and testing - native, SSH-remote, container, or CI - never where editing happens.
- **Record a narrowed platform and its reason in the repo** so the restriction reads as a deliberate dependency ceiling, not an omission. Nothing here is narrowed: the media tools this app orchestrates ship on all three platforms.
...
- [`PlexCleaner.code-workspace`](./PlexCleaner.code-workspace) carries the shared editor settings and the recommended-extension set. **All VS Code settings and extension recommendations live only there, never in a standalone `.vscode/settings.json` or `.vscode/extensions.json`** - [`.vscode/`](./.vscode/) holds only `tasks.json` and `launch.json`. The recommendation set is a standard part (markdown, spelling, EditorConfig, todo-tree, GitHub Actions) plus the language-specific part this repo needs (.NET, Python, Docker).
- The Table of Contents is maintained by the Markdown All in One extension, and `markdown.extension.toc.levels` in the workspace sets which heading levels it includes.
...
The GitHub repository details (the About panel) follow a fixed convention so the repo stays self-describing.

- **Description** matches the README's first non-empty line after the `#` H1 heading, as plain text - strip markdown links (`[text](url)` and `[text][ref]` become `text`) since a description is not rendered. The README is the source of truth: set the description from it (`gh api -X PATCH repos/<owner>/<repo> -f description=...`), never the reverse. When the current description is *more specific* than the README, surface the drift to the maintainer rather than silently discarding the detail - the fix is to sharpen the README so the description follows it.
