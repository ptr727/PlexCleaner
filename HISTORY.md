...
- Version 3.20:
  ...
  - Reworked the logging configuration and command line:
    ...
    - The log file now appends by default and rolls to a numbered file (`<name>_001.<ext>`) when it reaches a 1 GiB size limit; use `--logclear` to clear it before writing.
    ...
    - Moved logger construction into a static `LoggerFactory`, and re-leveled low-level tool-invocation and per-track diagnostic messages from `Information` to `Debug`/`Verbose` so the default output focuses on per-file actions and results. A Serilog `MinimumLevel.Override` escape hatch keeps the end-of-run summary and banner lines visible at any level.
