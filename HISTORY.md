...
- Version 3.20:
  ...
  - Reworked the logging configuration and command line:
    ...
    - Propagated the configured logger to the library dependencies (`InsaneGenius.Utilities` and `ptr727.LanguageTags`) so their output shares the same sinks and level, using a `Microsoft.Extensions.Logging` factory bridged from the Serilog logger (`Serilog.Extensions.Logging`).
