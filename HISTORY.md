...
- Version 3.21:
  - Stopped failing verification on non-monotonic DTS muxer warnings.
    ...
    - Verify now classifies the decode diagnostics: a result whose only diagnostics are the benign muxer timestamp warning passes, and anything else fails (fail-closed, so an unrecognized diagnostic still fails).
