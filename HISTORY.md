...
- Version 3.20:
  ...
  - Reworked the logging configuration and command line:
    ...
    - Propagated the configured logger to the library dependencies so their output shares the same sinks and level: `InsaneGenius.Utilities` via its Serilog logger, and `ptr727.LanguageTags` via a `Microsoft.Extensions.Logging` bridge (`Serilog.Extensions.Logging`).
