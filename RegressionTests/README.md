...
The seams for this already exist. Tool execution goes through the `MediaTool` wrappers over CliWrap, ultimately a `BufferedCommandResult` of stdout / stderr / exit code, and the parsed results live in `SidecarFile` as the ffprobe / mediainfo / mkvmerge JSON plus the `MediaProps` (`VideoProps` / `AudioProps` / `SubtitleProps` / `TrackProps`) the decision logic reads. So a test can either inject a canned `BufferedCommandResult` at the tool seam or construct a synthetic sidecar / `MediaProps` and drive the processing logic straight from it -- `FfMpegIdetParsingTests`, `VerifyClassifierTests`, and `ToolFailureLogFormatTests` (which mocks a `TestTool`) already exploit this seam; the goal is to apply it systematically across the processing paths.
